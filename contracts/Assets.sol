...
contract Assets is IERC721 {
    ...
    address private _coinsContractAddress;
    ...
    constructor (string memory name_, string memory symbol_, address coinsContractAddress_) {
        _name = name_;
        _symbol = symbol_;
        _coinsContractAddress = coinsContractAddress_;
    }
    ...
    function setPrice(uint _tokenId, uint _value) external { 
        require(ownerOf[_tokenId] != address(0), "Token invalid");
        require(ownerOf[_tokenId] == msg.sender, "Not a owner");
        price[_tokenId] = _value;
    }

    function swap(uint _tokenId) external returns (bool) {
        address owner = ownerOf[_tokenId];
        uint _price = price[_tokenId];
        require(_price > 0, "price not set");
        IERC20 coins = IERC20(_coinsContractAddress);
        require(coins.balanceOf(msg.sender) >= _price, "Not Enough Coins to swap");
        balanceOf[owner]--;
        balanceOf[msg.sender]++;
        ownerOf[_tokenId] = msg.sender;
        coins.transferFrom(msg.sender, owner, _price);
        return true;
    }
}
